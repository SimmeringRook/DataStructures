...
{
    public class UnorderedSingleLinkedList<T> : SingleLinkedList<T> where T : IComparable
    {
        public UnorderedSingleLinkedList() { }

        public UnorderedSingleLinkedList(IEnumerable<T> collection)
        {
            this.Add(collection);
        }
        ...
        public void Insert(int index, T valueToInsert)
        {
            if (index < 0 || index >= this.Count)
            {
                string exceptionMessage = (index < 0) ? " < 0" : " >= Count";
